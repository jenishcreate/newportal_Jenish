...
namespace newportal.Areas.Retailer.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        [HttpPost]
        public async Task<JsonResult> WalletRetailerTransacctionsJson(DateTime startDate, DateTime endDate)
        {
        ...
        public async Task<JsonResult> RechargeTransactionJson(DateTime startDate, DateTime endDate)
        {
            string? userid = User.FindFirstValue(ClaimTypes.NameIdentifier);
            
            IEnumerable<RechargeTransaction> objDataList = await _unitOfWork.Rechargetransaction.GetSelectedFieldsByUserIdAndDateRangeAsync(userid, startDate, endDate);
